...
class DayCountdown extends HTMLElement {
  ...
  set hass(hass) {
    ...
    this.content.innerHTML = `
      <br>
      <div align=center>
      <p style="font-size:20px;">${this.config.phrase}</p>
      <p style="font-size:35px;"><strong>${days}</strong></p>
      </div>
    `;
  } // end of HASS setter
  ...
  setConfig(config) {
    ...
    if (!this.config.phrase) {
      ...
      if (now < then) {
        this.config.phrase = "Days till Flower";
      } else if (then < now) {
        this.config.phrase = "Days of Flower"
      }
    }
